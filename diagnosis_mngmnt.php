...
if($_POST)
{
    ...
    $query = "SELECT  d.Diagnosis,d.year,d.Diagnosis_ID,pd.Patient_ID FROM diagnosis_tbl d join tbl_patient_diagnosis pd on pd.Diagnosis_ID=d.Diagnosis_ID where  CONCAT(`Diagnosis`, `year`) LIKE '%$valueToSearch%' order by d.year desc  ";
  ...
  /*for paging*/
  /*fetching data*/
 $sql="SELECT  d.Diagnosis,d.year,d.Diagnosis_ID,pd.Patient_ID FROM diagnosis_tbl d join tbl_patient_diagnosis pd on pd.Diagnosis_ID=d.Diagnosis_ID where pd.Patient_ID = '$patientID' order by d.year desc ";
...
if($currentpage > $lastpage){
  $currentpage=1;
   ...
   $offset = ($currentpage * 5) - $perpage;
    ...
    $query = "SELECT  d.Diagnosis,d.year,d.Diagnosis_ID,pd.Patient_ID FROM diagnosis_tbl d join tbl_patient_diagnosis pd on pd.Diagnosis_ID=d.Diagnosis_ID where pd.Patient_ID = '$patientID' order by d.year desc  limit $offset,$perpage ";
    $search_result = filterTable($query);
...
<body id="page-top">
  ...
  <a type="button" class="btn btn-secondary btn-info" title="Edit" href ="update_diagnosis_mngmnt.php?var_patient='.$row['Patient_ID'].'&var_diagnose='.$row['Diagnosis_ID'].'"><i class="fa fa-edit" aria-hidden="true" ></i></a>
  <a type="button" class="btn btn-secondary btn-danger" title="Remove" href ="delete_diagnosis_mngmnt.php?var_patient='.$row['Patient_ID'].'&var_diagnose='.$row['Diagnosis_ID'].'"><i class="fa fa-trash" aria-hidden="true" ></i></a>
    ';?>
                <tr>
